...
let entryPoint: {
  [key: string]: { price: number; date: Date | null; priceChanges: number };
} = {};
...
let testExitCondition = (ticker: string, price: number, indicator: number) => {
  ...
  if (
    entryPoint[ticker]?.price &&
    price > entryPoint[ticker]?.price * (1 + PROFIT_MARGIN)
  ) {
    console.log(
      `${ticker} exiting because of profit ${PROFIT_MARGIN * 100}% reached`,
    );
    return true;
  }
...
export class Basic implements StrategyExecution {
  async run(v: { new: Indicator }): Promise<void> {
    let obj = v.new;
    let price = obj.price;
    ...
    if (entryPoint[ticker]?.date) {
      if (testExitCondition(ticker, price, indicator)) {
        console.log(
          `Exit point ${ticker} ${entryPoint[ticker].price} -> ${price}`,
        );
        console.log(
          `Profit ${
            Math.round(((price / entryPoint[ticker].price) - 1) * 1000000) /
            10000
          }%`,
        );
      ...
      if (testEntryCondition(indicator)) {
        ...
        console.log(
          `Entry point ${ticker} ${entryPoint[ticker].price} ${
            entryPoint[ticker].date
          }`,
        );
